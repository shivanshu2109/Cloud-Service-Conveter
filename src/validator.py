"""
Intelligent Translation Validation System

Provides comprehensive quality assurance for cloud configuration translations
through hybrid AI and rule-based validation. Ensures accuracy, completeness,
and compliance with target cloud platform specifications.

Key Features:
- Dual-layer validation (AI + rule-based)
- Cross-cloud compliance verification
- Quantity and structure preservation checks
...
from utils import validate_required_keys, is_empty_value

class ValidationHandler:
    """
    Advanced validation engine for cloud configuration translations.
    
    Combines AI-powered analysis with rule-based validation to ensure translated
    configurations maintain structural integrity, preserve critical data, and
    comply with target cloud platform specifications.
    """
    
    def __init__(self, bedrock_client, config):
        """
        Initialize the validation system with AWS Bedrock client and configuration.
        
        Args:
            bedrock_client: AWS Bedrock runtime client for LLM interactions
            config: Configuration dictionary containing validation settings and prompts
        """
    ...
    def check_yaml_hierarchy_preservation(self, source_config, translated_config):
        ...
        # Check if all required keys are present using utils function
        missing_keys = validate_required_keys(translated_config, self.required_keys)
    ...
    def validate_with_llm(self, source_config, translated_config, source_cloud, target_cloud, model_info):
        ...
        # Include some manual checks for improved diagnostics using utils function
        ...
        # Ensure source_cloud and target_cloud are strings to prevent "dict has no attribute upper" error
